...
export const useDiagnostic = () => {
  ...
  const addSymptomMutation = useMutation({
    mutationFn: async (input: {
      symptom: { name: string; severity: string; historyId: string };
      username: string;
    }) => {
  ...
  const clearAllHistoryMutation = useMutation({
    mutationFn: async () => {
      const deletePromises = history.map((item) =>
        historyService.deleteHistory(item.id),
      );
  ...
  const addToSymptomList = (
    ...
    if (!name || name.trim().length === 0) {
      addToast('Symptom name is required', {
        type: 'warning',
    ...
    if (name.trim().length > 100) {
      addToast('Symptom name must be less than 100 characters', {
        type: 'warning',
        title: 'Validation Error',
      });
      return;
    ...
    if (symptoms.length >= 20) {
      addToast('Maximum 20 symptoms allowed', {
        type: 'warning',
        title: 'Limit Reached',
      });
      return;
    ...
    const isDuplicate = symptoms.some(
      (symptom) => symptom.name.toLowerCase() === name.trim().toLowerCase(),
    );

    if (isDuplicate) {
      addToast('This symptom has already been added', {
        type: 'warning',
    ...
    setNewSymptomName('');
    addToast(`Successfully added symptom: ${name.trim()}`, {
      type: 'success',
      duration: 2000,
    });
  };
  ...
  const removeFromSymptomList = (index: number) => {
    ...
    if (removedSymptom) {
      addToast(`Removed symptom: ${removedSymptom.name}`, {
        type: 'info',
        duration: 2000,
      });
    }
  ...
  const startDiagnostic = async (
    ...
    if (newDescription.trim().length < 10) {
      addToast('Description must be at least 10 characters long', {
        type: 'warning',
        title: 'Validation Error',
      });
      return;
    ...
    if (newDescription.trim().length > 500) {
      addToast('Description must be less than 500 characters', {
        type: 'warning',
        title: 'Validation Error',
      });
      return;
    ...
    if (selectedDate > today) {
      addToast('Date cannot be in the future', {
        type: 'warning',
        title: 'Validation Error',
      });
      return;
    ...
    if (symptoms.length === 0) {
      addToast('At least one symptom is required', {
        type: 'warning',
        title: 'Validation Error',
      });
      return;
    ...
    try {
      ...
      addToast('Starting comprehensive symptom analysis...', {
        type: 'success',
      });
      ...
      try {
        ...
        setTimeout(
          () => {
            ...
            addToast('Searching for suitable medications...', {
              type: 'success',
            });

            const drugsResponse =
              diagnosisResponse.recommendation_agent_response?.answer ||
              generateDrugsResponse(symptoms);
            ...
            setTimeout(() => {
              ...
              setTimeout(() => {
                ...
                addToast('Locating available products in marketplace...', {
                  type: 'success',
                });
                ...
                setTimeout(() => {
                  ...
                  setIsLoading(false);
                  addToast('Complete health analysis finished successfully!', {
                    type: 'success',
                  });
                }, 1000);
              }, 2000);
        ...
        console.warn(
          'Diagnosis service unavailable, using fallback:',
          diagnosisError,
        );

        const illnessResponse = generateIllnessResponse(
          newDescription,
          symptoms,
          newSince,
        );
      ...
      addToast('Failed to start diagnostic', {
        type: 'error',
  ...
  const generateIllnessResponse = (
    ...
    if (severeCounts > 0)
      response += `• ${severeCounts} severe symptom${severeCounts > 1 ? 's' : ''} detected\n`;
    if (moderateCounts > 0)
      response += `• ${moderateCounts} moderate symptom${moderateCounts > 1 ? 's' : ''} identified\n`;
    if (mildCounts > 0)
      response += `• ${mildCounts} mild symptom${mildCounts > 1 ? 's' : ''} noted\n`;
  ...
  const generateAIResponse = (
    ...
    response += `📊 Symptom Analysis:\n`;
    if (severeCounts > 0)
      response += `• ${severeCounts} severe symptom${severeCounts > 1 ? 's' : ''}\n`;
    if (moderateCounts > 0)
      response += `• ${moderateCounts} moderate symptom${moderateCounts > 1 ? 's' : ''}\n`;
    if (mildCounts > 0)
      response += `• ${mildCounts} mild symptom${mildCounts > 1 ? 's' : ''}\n`;
  ...
  const addToHistory = (
    ...
    addHistoryMutation.mutate({ assessment, username, diagnosis, title });
