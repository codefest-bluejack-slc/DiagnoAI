...
export const renderRichText = (text: string, colorScheme: 'purple' | 'blue' = 'purple') => {
  ...
  return processedText.split('\n').map((line, index) => {
    ...
    if (line.includes('*')) {
      ...
      processedLine = processedLine.replace(/\*\*([^*]+)\*\*/g, `<strong class="${colors.emphasis}">$1</strong>`);
      processedLine = processedLine.replace(/\*([^*]+)\*/g, `<em class="${colors.emphasis}">$1</em>`);
