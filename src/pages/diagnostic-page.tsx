...
import { getSeverityColor, getStepIcon, getSeverityIcon } from '../utils/diagnostic-utils';
                  ...
                  <div className="text-center">
                    <div className="relative w-24 h-24 mx-auto mb-4">
                      ...
                      <div className="absolute inset-0 flex items-center justify-center">
                        <span className="text-white font-bold text-lg">{symptoms.length}</span>
                      </div>
                    </div>
                    <p className="text-purple-200 text-sm font-medium">Symptoms Recorded</p>
                    <p className="text-purple-300 text-xs mt-1">Up to 8 recommended</p>
                  </div>
                </div>
                ...
                <div className="step-card">
                  ...
                  <div className="space-y-3">
                    {['input', 'review', 'analysis'].map((step, index) => (
                      <div key={step} className={`flex items-center gap-3 p-3 rounded-lg transition-all duration-300 ${
                        currentStep === step ? 'step-item-active' : 'step-item-inactive'
                      }`}>
                        <div className={`p-2 rounded-full ${
                          currentStep === step ? 'bg-purple-500/30' : 'bg-white/10'
                        }`}>
                          {getStepIcon(step)}
                        </div>
                        <div>
                          <p className={`font-medium text-sm ${
                            ...
                            {step === 'input' ? 'Input Symptoms' : step === 'review' ? 'Review & Confirm' : 'AI Analysis'}
                          </p>
                          <p className="text-purple-300 text-xs">
                            {step === 'input' ? 'Describe your symptoms' : step === 'review' ? 'Verify information' : 'Get insights'}
                          </p>
                        </div>
                      </div>
                    ))}
                  ...
                  >
                    ...
                    <div className="relative text-center">
                      <div className="w-20 h-20 mx-auto mb-6 bg-gradient-to-br from-purple-500/20 to-indigo-500/20 rounded-full flex items-center justify-center group-hover:scale-110 transition-transform duration-500">
                        <Plus className="text-purple-300 group-hover:rotate-90 transition-transform duration-300" size={32} />
                      </div>
                      <h3 className="text-2xl font-semibold text-white mb-3 group-hover:text-purple-200 transition-colors duration-300">Add New Symptom</h3>
                      <p className="text-purple-200 mb-6">Click here to describe what you're experiencing</p>
                  ...
                  <div className="symptom-form-card animate-in slide-in-from-top-5 fade-in duration-500">
                    <div className="flex items-center justify-between mb-6">
                      <h3 className="text-2xl font-semibold text-white flex items-center gap-3">
                        ...
                        <span className="animate-in slide-in-from-left-3 duration-300 delay-200">Describe Your Symptom</span>
                      </h3>
                      <button
                  ...
                  <div className="symptoms-list-card animate-in fade-in slide-in-from-bottom-3 duration-500">
                    <div className="flex items-center justify-between mb-6">
                      ...
                      {!showAddForm && (
                        ...
                        >
                          <Plus className="text-purple-300 hover:rotate-90 transition-transform duration-300" size={18} />
                        </button>
                      )}
                    </div>
                          ...
                          <div className="flex items-start justify-between gap-3">
                            <div className="flex-1">
                              ...
                              {(symptom.severity || symptom.duration) && (
                                <div className="flex items-center gap-3 text-sm mt-3 animate-in fade-in duration-300" style={{ animationDelay: `${index * 150 + 300}ms` }}>
                                  {symptom.severity && (
                                    <span className={`inline-flex items-center gap-1 px-3 py-1 rounded-full border transition-all duration-300 hover:scale-105 ${getSeverityColor(symptom.severity)}`}>
            ...
            <div className="col-span-12 lg:col-span-3">
              <div className="sticky top-24 space-y-6">
                ...
                <div className="tips-card">
                  ...
                  <div className="space-y-3">
                    {[
                      { icon: Edit3, tip: "Be specific about location and intensity" },
                      { icon: Clock, tip: "Include duration and frequency" },
                      { icon: Search, tip: "Mention triggers or patterns" },
                      { icon: Lightbulb, tip: "Add at least 3 symptoms for better analysis" }
                    ].map((item, index) => (
                      <div key={index} className="flex items-start gap-3 p-3 bg-white/5 rounded-lg">
                        <div className="p-1 bg-purple-500/20 rounded">
                          <item.icon size={16} className="text-purple-300" />
                        </div>
                        <p className="text-purple-200 text-sm leading-relaxed">{item.tip}</p>
                      </div>
                    ))}
                  </div>
                </div>
                  ...
                  <div className="warning-card">
                    <div className="text-center">
                      ...
                      <p className="text-amber-200 text-sm font-medium mb-2">
                        Add {3 - symptoms.length} more symptom{3 - symptoms.length !== 1 ? 's' : ''}
                      </p>
