...
export class DiagnosisService {
  ...
  public static async getStructuredDiagnosis(
    ...
    for (let attempt = 1; attempt <= maxRetries; attempt++) {
      try {
        console.log(
          `Diagnosis API attempt ${attempt}/${maxRetries} to ${this.BASE_URL}/diagnosis/from-symptoms`,
        );

        const response = await axios.post(
          `${this.BASE_URL}/diagnosis/from-symptoms`,
          request,
          {
            headers: {
        ...
        if (
          ...
          (response.data.diagnosis ||
            response.data.recommendation_agent_response)
        ) {
        ...
        console.warn(
          `Attempt ${attempt}: Response missing required data fields`,
        );
        ...
        if (error.response?.status === 500 && error.response?.data) {
          try {
            ...
            if (
              error.response.data.diagnosis ||
              error.response.data.recommendation_agent_response
            ) {
...
- Keep a record of symptom changes for your healthcare provider`,
      recommendation_agent_response: {
        answer:
          'Due to technical limitations, specific medication recommendations are not available at this time. Please consult with a healthcare professional or pharmacist for appropriate medication guidance based on your symptoms.',
        medicines: [],
      },
    };
  }

  public static async getUnstructuredDiagnosis(
    request: DiagnosisRawRequest,
  ): Promise<DiagnosisFromSymptomsRequest> {
    if (this.USE_TEST_MODE) {
      return new Promise((resolve) => {
        setTimeout(() => {
          resolve({
    ...
    try {
      const response = await axios.post(
        `${this.BASE_URL}/diagnosis/get_structure`,
        request,
        {
          headers: {
            'Content-Type': 'application/json',
      ...
      console.log(
        'response from diagnosis service (LANGSUNG DARI API) ',
        response.data.result,
      );

      const result = response.data.result;
      const parsedData: IDiagnostic =
