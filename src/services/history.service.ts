...
import {
  _SERVICE as HistoryCanisterService,
  HistoryResponse,
  History,
} from '../declarations/history/history.did';
import {
  canisterId as historyCanisterId,
  createActor as createHistoryActor,
} from '../declarations/history';
...
export class HistoryService extends BaseService<HistoryCanisterService> {
  ...
  public async getHistoryById(id: string): Promise<HistoryResponse | null> {
    try {
      const response = await this.actor.getHistoryWithSymptoms(
        id,
        symptomCanisterId,
      );
    ...
    try {
      ...
      if ('ok' in response) {
        const historyPromises = response.ok.map(async (history) => {
          try {
            const historyWithSymptoms = await this.actor.getHistoryWithSymptoms(
              history.id,
              symptomCanisterId,
            );
            ...
            console.error(
              `Error fetching symptoms for history ${history.id}:`,
              error,
            );
          }
  ...
  public async updateHistoryWithDiagnosis(
    id: string,
    diagnosis: string,
  ...
  public async updateHistory(
    id: string,
    updates: Partial<IHealthAssessment>,
  ): Promise<boolean> {
    try {
      const principal = await BaseService.getCallerPrincipal();
      if (principal.isAnonymous()) {
