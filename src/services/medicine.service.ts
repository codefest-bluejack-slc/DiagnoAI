...
export const searchProducts = async (
  ...
  try {
        ...
        shopping_results: data.shopping_results
          ...
          .map((product: any, index: number) => {
            const enhancedProduct = {
              ...
              product_id:
                product.product_id ||
                product.link?.split('?')[0]?.split('/').pop() ||
                `${Date.now()}_${Math.random()}_${index}`,
            };
            ...
            const productDetails: IProductDetails = {
              ...
              description:
                product.snippet ||
                product.description ||
                `${product.title} - Available from ${product.source}. Visit the product page for full details and customer reviews.`,
              ...
              shipping_info: {
                time:
                  product.delivery || product.shipping || 'Standard delivery',
              ...
              product_highlights: product.highlights ||
                product.features || [
                  ...
                  `Price: ${product.price}`,
                  ...(product.rating
                    ? [`Customer Rating: ${product.rating} stars`]
                    : []),
                  ...(product.reviews
    ...
    throw new Error(
      `CORS proxy failed: ${error instanceof Error ? error.message : 'Unknown error'}`,
    );
  }
};
...
export const getProductDetails = async (
  ...
  if (cachedProduct) {
    const productDetails: IProductDetails = {
      ...
      availability:
        (cachedProduct as any).availability || 'Available for purchase',
      shipping_info: {
      ...
      reviews_data: {
        ...
        recent_reviews:
          cachedProduct.reviews &&
          cachedProduct.reviews > 0 &&
          cachedProduct.rating
            ? [
                {
      ...
      product_highlights: (cachedProduct as any).highlights ||
        (cachedProduct as any).features || [
          ...
          ...(cachedProduct.rating
            ? [`Customer Rating: ${cachedProduct.rating} stars`]
            : []),
          ...(cachedProduct.reviews
            ? [`${cachedProduct.reviews} customer reviews`]
            : []),
        ],
